...
- **云剪贴板（可选，默认关闭）**：在设置中配置 WebDAV 地址、用户名与密码（密码加密保存在本机）。开启后，仅在输入法窗口可见时，将您复制的**纯文本**按最小间隔上传至您指定的 WebDAV 目录（默认 `moqi-input-method/`）。系统标记为敏感的内容、掩码密码样式等会尽力跳过，避免与下载上屏形成循环上传。
